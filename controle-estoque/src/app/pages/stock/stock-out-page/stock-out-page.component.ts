...
import { OUT } from 'src/app/constants/movement-constants';
import { Batch, Entity, Movement, Product, Quantity, Location } from 'src/app/entity/Entities';
...
@Component({
    selector: 'app-stock-out-page',
    templateUrl: './stock-out-page.component.html',
    styleUrls: ['./stock-out-page.component.css']
})
export class StockOutPageComponent implements OnInit {
    products: Product[] = [];
    locations: Location[] = [];
    batches: Batch[] = [];
    ...
    saveOut(): void {
        if (confirm('Deseja salvar a movimentação de saída?')) {
            if (this.isValidMovement() && this.productModel && this.locationModel && this.batchModel) {
                if (this.isValidMovement() && this.productModel && this.locationModel && this.batchModel) {
                    let movement: Movement = new Movement(this.productId, this.batchId, OUT, this.quantity);
                    movement.name = this.productModel.name;
                    movement.locationFrom = this.locationId;
                    movement.locationFromName = this.locationModel.name;
